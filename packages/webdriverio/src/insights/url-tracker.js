...
const { ApiUploader } = require("@lambdatest/sdk-utils");
...
try {
  ...
  console.warn(
    "HTML Reporter not available. Install @lambdatest/sdk-utils for HTML reports."
  );
}
...
function resetTrackingFileOnLoad() {
  ...
  try {
    ...
    console.log(
      `[UrlTracker] Resetting tracking file at module load: ${outputPath}`
    );
    ...
    console.log(
      `[UrlTracker] Error resetting file on module load: ${error.message}`
    );
    return false;
  }
}
...
class UrlTracker extends EventEmitter {
  constructor(browser, options = {}) {
    ...
    if (this.options.enableApiUpload) {
      logger.info(
        "API upload is enabled for WebDriverIO URL tracker, initializing API uploader..."
      );
  ...
  processLocationHistory(browserHistory) {
    ...
    this.log(
      `Processing browser location history: ${browserHistory.length} entries`
    );
  ...
  saveReport() {
    try {
        ...
        try {
          ...
          if (fileContent.trim() !== "") {
            try {
              ...
              this.log(
                `Error parsing existing data, creating new file: ${e.message}`
              );
              existingData = [];
              fs.writeFileSync(outputPath, "[]", "utf8");
            }
      ...
      const existingReportIndex = existingData.findIndex(
        (r) =>
          r.spec_file === report.spec_file &&
          r.test_name === report.test_name &&
          r.session_id === report.session_id
      );
      ...
      try {
        ...
        this.log(
          `Report saved with ${report.navigation_count} navigation events`
        );
  ...
  getTrackingResults() {
    logger.info(
      `Getting ${this.trackingResults.length} tracking results for test: ${
  ...
  async cleanup() {
    ...
    if (this.cleanupCalled) {
      logger.info(
        `Cleanup already called for test: ${this.currentTestName}, skipping`
      );
      return;
    }
    ...
    logger.info(
      `API upload enabled: ${this.options.enableApiUpload} (type: ${typeof this
        .options.enableApiUpload})`
    );
    logger.info(`API uploader exists: ${!!this.apiUploader}`);
    logger.info(
      `Tracking results count: ${
        this.trackingResults ? this.trackingResults.length : 0
      }`
    );
    ...
    if (
      ...
      logger.info(
        "Converting navigationHistory to trackingResults for API upload"
      );
    ...
    if (
      ...
      try {
        ...
        logger.info(
          `Uploading ${this.trackingResults.length} tracking results`
        );
        ...
        if (ApiUploader.validateTrackingData(trackingData, "url-tracker")) {
          ...
          // Upload to API
          const response = await this.apiUploader.uploadTrackingResults(
            trackingData,
            testId,
            {
    ...
    if (
      ...
      try {
        ...
        const htmlReportPath = htmlReporter.generateReport(
          sessionData,
          "webdriverio"
        );
    ...
    try {
      ...
      if (fs.existsSync(outputPath)) {
        ...
        try {
          const content = fs.readFileSync(outputPath, "utf8");
          this.log(
            `File content: ${content.substring(0, 100)}${
              content.length > 100 ? "..." : ""
            }`
          );
