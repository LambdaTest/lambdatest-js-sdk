...
class HtmlReporter {
    ...
    constructor(options: HtmlReporterOptions = {}) {
        this.options = {
            ...
            ...options
        } as Required<HtmlReporterOptions>;
     ...
     */
    generateReport(trackingData: any, framework: string = 'unknown'): string {
        try {
     ...
     */
    private parseTrackingData(data: any, framework: string): ReportData {
        ...
        try {
            ...
            normalized.summary.totalNavigations = normalized.sessions.reduce((total: number, session: SessionData) => {
                return total + (session.navigations ? session.navigations.length : 0);
            }, 0);
            
            normalized.summary.testFiles = [...new Set(normalized.sessions.map((s: SessionData) => s.spec_file).filter(Boolean))];
     ...
     */
    private normalizeSession(session: any, framework: string): SessionData {
        ...
        return {
            ...
            navigations: navigations.map((nav: any) => this.normalizeNavigation(nav, framework)),
            framework
        };
    }
     ...
     */
    private normalizeNavigation(navigation: any, framework: string): NavigationItem {
     ...
     */
    private generateHtmlContent(reportData: ReportData, framework: string): string {
...
<body class="${isDark ? 'dark' : 'light'}">
    ...
    private getStyles(isDark: boolean): string {
        const colors: ColorTheme = isDark ? {
            bg: '#0d1117',
            surface: '#161b22',
            surfaceHover: '#21262d',
            text: '#f0f6fc',
     ...
     */
    private generateHeader(reportData: ReportData): string {
        return `
            <div class="header">
     ...
     */
    private generateSessions(reportData: ReportData): string {
        ...
        const sessionsHtml = reportData.sessions.map((session: SessionData, index: number) => `
     ...
     */
    private generateNavigations(navigations: NavigationItem[], framework: string): string {
        if (!navigations || navigations.length === 0) {
            return '<div class="empty-state"><p>No navigations recorded</p></div>';
        }

        return navigations.map((nav: NavigationItem, index: number) => {
     ...
     */
    static generateFromFiles(options: HtmlReporterOptions = {}): string | null {
        ...
        trackingFiles.forEach((filePath: string) => {
            try {
                ...
                if (filePath.includes('navigation-tracking')) {
                    ...
                    detectedFramework = data.some((session: any) => session.metadata) ? 'playwright' : 'webdriverio';
                }
    ...
    findTrackingFiles(): string[] {
        ...
        return possibleFiles.filter((filePath: string) => {
            return fs.existsSync(filePath) && fs.statSync(filePath).size > 0;
        });
    }
}

export { HtmlReporter, HtmlReporterOptions, NavigationItem, SessionData, ReportData, ReportSummary }; 